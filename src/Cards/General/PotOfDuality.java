...
public class PotOfDuality extends SpellCard {
    public PotOfDuality() {
        super("强欲而谦虚之壶", "通常", new ArrayList<>());
    }

    @Override
    public boolean usableEffect(Set<String> cardsUsed){
        RestrictionChecker restriction = new RestrictionChecker(cardsUsed);
        if(cardsUsed.contains(name) || restriction.specialSummoned){
            return false;
        }
        return true;
    }
        ...
        for (Card card : filteredRevealedCards) {
            ...
            for (Card revealedCard: revealedCards) {
                if (revealedCard != card) {
                    newHandDeck.addCard(revealedCard);
                }
            }
